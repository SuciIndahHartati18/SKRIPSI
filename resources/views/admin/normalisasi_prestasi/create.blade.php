<x-layout>
    ...
    <div class="bg-slate-200 flex flex-col px-4 py-4 gap-3 shadow shadow-slate-500">
        ...
        @if ($selectedSiswa)
            <form method="POST" action="{{ route('admin.normalisasi_prestasi.store') }}">
                <div class="flex flex-col gap-3">
                    ...
                    @foreach ($kriteriaPrestasi as $kriteria)
                        <x-form.container variant="label-input">
                            <x-form.label for="">{{ $kriteria->nama_kriteria_prestasi }}</x-form.label>
                            <x-form.input type="number" step="0.01"
                                name="nilai_normalisasi_prestasi[{{ $kriteria->id }}]"
