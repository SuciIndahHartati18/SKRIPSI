<x-layout-dashboard>
    ...
    <div class="bg-slate-200 flex flex-col px-4 py-4 gap-3 shadow shadow-slate-500">
        <form method="GET" action="{{ route('admin.hasil_seleksi_tes.editRanking') }}">
        ...
        @if ($hasilSeleksi->count())
            <form method="POST" action="{{ route('admin.hasil_seleksi_tes.updateRanking') }}">
                @csrf
                @method('PUT')
                ...
                <div class="flex flex-col gap-3">
                    ...
                    <table>
                        <thead>
                            <tr class="bg-rose-900">
                                ...
                                <th class="border border-slate-500 text-slate-100 px-3 py-1 whitespace-nowrap">Nilai Akhir Tes</th>
                            </tr>
                        </thead>
                        <tbody>
                            @foreach ($hasilSeleksi as $item)
                                <tr>
                                    ...
                                    <td class="border border-slate-500 text-center px-3 py-1 whitespace-nowrap">{{ $item->nilai_akhir_tes }}</td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
                    ...
                    <div class="flex gap-2 justify-end">
                        <a href="{{ route('admin.perhitungan_jalur_tes.index') }}" type="submit" class="inline-block bg-red-500 hover:bg-red-700 font-bold text-slate-100 text-center px-4 py-1 rounded rounded-sm">
                            Batal
                        </a>
        ...
        @elseif($tahunDipilih)
            <p class="mt-4 text-red-600">Tidak ada data hasil seleksi untuk tahun ajaran {{ $tahunDipilih }}.</p>
        @endif
    </div>
</x-layout-dashboard>